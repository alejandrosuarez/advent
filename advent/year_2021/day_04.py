...
22 11 13  6  5
 2  0 12  3  7
"""
ANSWERS = [4512, 25410, 1924, 2730]


def _transform_groups(groups):
    numbers = [int(num) for num in groups[0].split(",")]
    locations = cl.defaultdict(list)
    boards = []
    for b, board in enumerate(groups[1:]):
        boards.append([])
        for r, row in enumerate(board.split("\n")):
            boards[b].append([])
            for c, val in enumerate(row.split()):
                boards[b][r].append(int(val))
                locations[int(val)].append((b, r, c))
    return numbers, locations, boards
