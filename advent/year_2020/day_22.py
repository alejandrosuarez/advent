...
Fortunately, you brought a small deck of space cards! You'd like to play a game of Combat,
...
Once the game ends, you can calculate the winning player's score. The bottom card in their deck
is worth the value of the card multiplied by 1, the second-from-the-bottom card is worth the value of the
card multiplied by 2, and so on. With 10 cards, the top card is worth the value on the card multiplied by 10.
...
starting decks as before - it's only fair). Then, the game consists of a series of rounds with a few changes:
...
is above the other card). Note that the winner's card might be the lower-valued of the two cards if
they won the round due to winning a sub-game. If collecting cards by winning the round causes a player to have
all of the cards, they win, and the game ends.
...
Recursive Combat. (For example, if player 1 draws the 3 card, and player 2 draws the 7 card,
...
During this sub-game, the game that triggered it is on hold and completely unaffected; no cards are
removed from players' decks to form the sub-game. (For example, if player 1 drew the 3 card, their deck in
the sub-game would be copies of the next three cards in their deck.)
...
original deck using the same rules as regular Combat. In the above game, the winning player's score is 291.
...
def _pt1(groups):
    ...
    return _winning_score(p1, p2)


def _encode_decks(x, y):
    strs = [
        ",".join(map(str, x)),
        ",".join(map(str, y)),
    ]
...
def _round(p1, p2):
    c1, c2, w = p1.popleft(), p2.popleft(), None
    upto = lambda q, c: cl.deque(list(q)[:c])

    if len(p1) >= c1 and len(p2) >= c2:
        n1, n2 = _game(upto(p1, c1), upto(p2, c2))
        w = 1 if n1 and not n2 else 2
    else:
        w = 1 if c1 > c2 else 2
