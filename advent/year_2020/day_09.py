...
any two of the 25 immediately previous numbers. The two numbers will have different values,
...
To find the encryption weakness, add together the minq and maxq number in this contiguous range;
...
def _pt1(nums, preamble):
    ...
    for i in range(preamble, len(nums)):
        ...
        for c, j in compliments.items():
            if k - c in compliments and compliments[k - c] != j:
                break
        else:
            return k
