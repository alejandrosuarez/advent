...
in your list and proceeding one by one until one million is reached. (For example,
if your labeling were 54321, the cups would be numbered 5, 4, 3, 2, 1, and then start counting up
...
class ListNode:
    ...
    nxt: t.Optional[ListNode] = None


def _remove_cups(node):
    head = last = ListNode()
    rems = []
    ...
    last.nxt = None
    return head.nxt, last, rems


def _list_values(node, n):
    result = []
...
def _build_circular_linked_list(arr):
    idx = {n: ListNode(n) for n in arr}
    minval = maxval = arr[0]
    cur = head = idx[arr[0]]
    ...
    cur.nxt = head
    return head, idx, minval, maxval


def _run(arr, n, first=None):
    cur, idx, minval, maxval = _build_circular_linked_list(arr)

    for i in range(n):
        remstart, remend, rems = _remove_cups(cur)
        destval = cur.val - 1 if cur.val != minval else maxval

        while destval in set(rems):
            destval = destval - 1 if destval > minval else maxval
    ...
    return _list_values(idx[1].nxt, first or len(arr) - 1)


def _pt1(arr):
    fmt = lambda a: "".join(list(map(str, a)))
    return (fmt(_run(arr, 10)), fmt(_run(arr, 100)))


def _pt2(arr):
    n = 1_000_000
    arr += list(range(max(arr) + 1, n + 1))
    cups = _run(arr, n * 10, first=2)
    return math.prod(cups)
