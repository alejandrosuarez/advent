...
Your flight to the major airline hub reaches cruising altitude without incident. While you consider checking the in-flight menu for
one of those drinks that come with a little umbrella, you are interrupted by the kid sitting next to you.
...
jmp jumps to a new instruction relative to itself. The next instruction to execute is found using the argument as an offset
from the jmp instruction; for example, jmp +2 would skip the next instruction, jmp +1 would continue to the instruction
...
other acc +1 near the bottom. After it increases the accumulator from 1 to 2, jmp -4 executes, setting the next
instruction to the only acc +3.
...
acc increase global accl

when cycle is detected terminate dfs (pt1)
...
def _dfs(pos, acc, ins, color, fix):
    if pos == len(ins):
        return True, acc

    if color[pos] == VISITING:
        return False, acc

    color[pos] = VISITING
    cmd, arg = ins[pos]
    rest, term, result = [ins, color, fix], True, acc

    if cmd == ACC:
        term, result = _dfs(pos + 1, acc + int(arg), *rest)
    elif cmd == NOP:
        term, result = _dfs(pos + 1, acc, *rest)
        if fix and not term:
            term, result = _dfs(pos + int(arg), acc, *rest)
    elif cmd == JMP:
        term, result = _dfs(pos + int(arg), acc, *rest)
        if fix and not term:
            term, result = _dfs(pos + 1, acc, *rest)
...
def main():
    pt1 = lambda ins: _dfs(0, 0, ins, [0] * len(ins), False)[1]
