...
(0 through 63) or the back (64 through 127). The next letter indicates which half of that
...
pass in your list. However, there's a catch: some of the seats at the very front and back of
...
def _sid(l):
    r = _bsearch(0, 127, l[:7].strip(), "B", "F")
    c = _bsearch(0, 7, l[7:].strip(), "R", "L")
    return r * 8 + c
...
def _solve(sids):
    ...
    for i in range(1, len(sids)):
        if sids[i] - sids[i - 1] > 1:
            return sids[-1], sids[i] - 1


def main():
    return afs.input_lines(parts=[_solve], transform_line=_sid)