...
compatible with the docking program on the ferry, so the docking parameters aren't being correctly initialized in the docking program's memory.
...
def pt2(instructions):
    ...
    def backtrack(mask, i, addr, val):
        if i == len(mask):
            ...
            backtrack(mask, i + 1, addr, val)
        else:
            if mask[i] == "X":
                backtrack(mask, i + 1, addr & ~(1 << i), val)
            backtrack(mask, i + 1, addr | 1 << i, val)

    for mask, writes in instructions:
        for addr, val in writes:
            backtrack(mask[::-1], 0, addr, val)
