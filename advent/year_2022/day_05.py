...
crushed or fall over, the crane operator will rearrange them in a series of carefully-planned steps.
...
def _parse(grps):
    stacks, drs = grps
    drs = [d.split(" ") for d in drs.splitlines()]
    drs = [(int(d[1]), int(d[3]), int(d[5])) for d in drs]
    stks = [list(s) for s in stacks.splitlines()]
    stks = np.array([list(reversed(s)) for s in np.transpose(stks, (1, 0))])
    stks = [list(s) for s in stks if s[0].isnumeric()]
    stks = {int(s[0]): [a for a in s[1:] if a.strip()] for s in stks}
    return stks, drs


def _pt1(grps):
    stacks, drs = grps
    for (qty, src, dest) in drs:
        for _ in range(qty):
...
move 1 from 1 to 2
"""
ANSWERS = ["CMZ", "VGBBJCRMN", "MCD", "LBBVJBRMH"]
