...
def _sliding_window(txt: str, size: int) -> int:
    counts = cl.defaultdict(int)
    for i, c in enumerate(txt):
        counts[c] += 1
        if i >= size:
            counts[txt[i - size]] -= 1
            if not counts[txt[i - size]]:
                del counts[txt[i - size]]
        if len(counts) == size:
            return i + 1


def _pt1(lines):
    return _sliding_window(lines.pop(), 4)


def _pt2(lines):
    return _sliding_window(lines.pop(), 14)
...
"""
ANSWERS = [7, 5, 6, 10, 11, 1760, 19, 23, 23, 29, 26, 2974]
