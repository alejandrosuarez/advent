...
list of the dimensions (length l, width w, and height h) of each present, and only want to order exactly as much as they need.
...
def _pt1(lines):
    def wrapping(lwh):
        l, w, h = lwh
        return (2 * l * w) + (2 * w * h) + (2 * h * l) + ((l * w * h) // max(lwh))

    return sum(map(wrapping, lines))


def _pt2(lines):
    def ribbon(lwh):
        l, w, h = lwh
        return (l * w * h) + ((2 * l + 2 * w + 2 * h) - 2 * max(lwh))

    return sum(map(ribbon, lines))
...
def main():
    return afs.input_lines(
        tests=[TEST],
        parts=[_pt1, _pt2],
        transform_line=lambda l: [int(n) for n in l.split("x")],
    )
