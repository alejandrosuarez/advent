...
def _nice1(string):
    vowels = len(re.findall(r"[aeiou]", string)) >= 3
    repeated = re.search(r"([a-z])\1", string)
    clean = not re.search(r"ab|cd|pq|xy", string)

    return all([vowels, repeated, clean])


def _nice2(string):
    repeated = re.search(r"([a-z])[a-z]\1", string)
    pair = re.search(r"([a-z]{2})[a-z]*\1", string)

    return all([repeated, pair])
...
def main():
    pt1 = lambda l: sum(map(_nice1, l))
    pt2 = lambda l: sum(map(_nice2, l))
